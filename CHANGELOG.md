...
- [Allow using `roomNames` to override the room names retrieved from the vacuum](TBD): Some RVCs return long numeric names to the rooms instead of the actual names configured in the app. This change allows to override those retrieved names via the existing `roomNames` setting.
